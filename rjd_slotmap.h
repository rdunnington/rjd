...
void* rjd_slotmap_alloc_impl(size_t sizeof_type, size_t count, struct rjd_mem_allocator* allocator);
...
static void* rjd_slotmap_grow(void* oldmap, uint32_t count, size_t sizeof_type, struct rjd_mem_allocator* allocator)
...
void* rjd_slotmap_insert_impl(void* map, struct rjd_slot* out_slot)
{
	...
	if (rjd_array_count(header->freelist) == 0)
		map = rjd_slotmap_grow(map, header->count * 2, header->sizeof_type, header->);
...
uint32_t rjd_slotmap_get_impl(const void* map, struct rjd_slot slot)
{
	...
	RJD_ASSERTMSG(!rjd_array_contains(header->freelist, (uint32_t)slot.index), "This slot is unallocated.");
	return slot.index;
}
...
void rjd_slotmap_erase(void* map, struct rjd_slot slot)
{
	RJD_ASSERT(map);
	const struct rjd_slotmap_header* header = rjd_slotmap_getheader(map);
	RJD_ASSERT(slot.index < header->count);
...
void* rjd_slotmap_grow(void* oldmap, uint32_t count, size_t sizeof_type, struct rjd_mem_allocator* allocator)
{
	...
	struct rjd_slotmap_header* header = (struct rjd_slotmap_header* header)mem;
	header->allocator = allocator;
	header->count = count;
	header->data = (void*)(mem + sizeof(struct rjd_slotmap_header));
	header->salts = (uint16_t*)((char*)data + sizeof_type * count);
